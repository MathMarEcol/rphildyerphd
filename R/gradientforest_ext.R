...
#' @param clara_args list of additional arguments to cluster::clara
#'
...
gf_clust_f_ratio <- function(gf,
  ...
  cluster_fratio <- lapply(cluster_list, spatial_vars = spatial_vars, function(clust, spatial_vars){
    ...
    #cluster of samples
    node_clust <- data.frame(leaf = mvpart_result, clust = clust$clustering[gf_grid_sites])
    ret$conf <- rphildyerphd:::opt_confusion(node_clust$leaf, node_clust$clust)
    ret$anova <- gf_anova(gf, k = length(clust$i.med), node_clust$clust)
...
#'
#' @param mvpart_map vector of mvpart leaf id's for each grid cell
#' @param clust_map vector of k-medoid/k-means cluster id's for each grid cell
#'
...
#' #reattach spatial data
#' env_trans <- cbind(env_grid[names(env_grid) %in% spatial_vars], env_trans)
#' node_clara <- data.frame(leaf = test3$mvpart, clust = test3$clust_list[[3]]$clustering[gf_grid_sites])
...
#'   node_clara <- data.frame(leaf = test3$mvpart, clust = clust$clustering[gf_grid_sites])
